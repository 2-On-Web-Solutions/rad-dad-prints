...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: row, error: selErr } = await supabaseAdmin
      .from('design_images')
      .select('id, design_id, image_url, storage_path')
      .eq('id', id)
      .single<Row>();
    ...
    if (row.storage_path) {
      // If you saved something like "<folder>/<filename>"
      // set your real bucket name here:
    ...
    if (bucket && path) {
      const { error: delStorageErr } = await supabaseAdmin.storage
        ...
        .remove([path]);
      if (delStorageErr) {
        // Log but do not fail the whole request; we still remove DB row.
        console.warn('Storage remove warning (image):', delStorageErr.message);
      }
    }
