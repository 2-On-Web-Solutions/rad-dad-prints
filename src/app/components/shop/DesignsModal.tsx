...
export default function DesignsModal({
  ...
  const [items, setItems] = useState<PublicDesignLite[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const url = `/api/public/designs${sp.toString() ? `?${sp.toString()}` : ''}`;
        const res = await fetch(url, { cache: 'no-store' });
        if (!res.ok) {
          const txt = await res.text().catch(() => '');
          console.error('Designs API error', res.status, txt);
  ...
  async function openDetail(id: string) {
    ...
    try {
      const res = await fetch(`/api/public/designs/${id}`, { cache: 'no-store' });
      if (!res.ok) {
        console.error('detail api error', res.status);
        return;
      }
      const d = await safeJson(res) as PublicDesignFull;

      // thumbnail first, then bucket images (deduped)
    ...
    >
      <div className="relative w-full max-w-6xl bg-[var(--color-background)] text-[var(--color-foreground)] rounded-xl shadow-2xl
        ...
        <div className="flex-1 overflow-y-auto" style={{ scrollbarGutter: 'stable' }}>
          ...
          >
            {loadingDesigns && Array.from({ length: pageSize }).map((_, i) => (
            ...
            {!loadingDesigns && items.map((it) => (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  {it.price_from && <div className="text-sm text-teal-400 mt-1">{it.price_from}</div>}
                  {view === 'cards' && it.blurb && (
          ...
          <div className="px-5 md:px-8 pb-6">
            <p className="text-sm opacity-70">
              Tip: These designs are examples. We can customize sizing, colors, and materials for your project.
            </p>
          </div>
        </div>
      </div>
