...
export default function BundlesItemModal({
  open,
  item,
  onClose,
}: {
  ...
  // Compose image list (fallback to single img if provided)
  const imgs =
  ...
  // reset on open / when switching to a new item
  useEffect(() => {
    if (open) setIdx(0);
  }, [open, item?.id]);

  // ESC closes, ←/→ navigate (deps array stays the same size/order)
  useEffect(() => {
    if (!open) return;
      ...
      <div className="relative w-full max-w-5xl bg-[var(--color-background)] text-[var(--color-foreground)] rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[100px_1fr_260px] gap-4 md:gap-6 px-6 py-6">
          ...
          <div className="flex flex-col justify-between md:h-[380px] mt-4 md:mt-0">
            <div>
              {item.priceFrom && (
                <div className="text-teal-400 font-semibold mb-1">{item.priceFrom}</div>
              )}
              {item.blurb && (
            ...
            <div className="mt-4 md:mt-auto">
              {item.files?.length ? (
                ...
                <div className="opacity-70 text-sm mt-2">
                  No downloads yet. Contact us for custom export.
                </div>
              )}
            </div>
          </div>
